...
function casper_custom_header_and_background() {
...
add_action( 'after_setup_theme', 'casper_custom_header_and_background' );
...
function casper_header_style() {
   ...
   $header_image = get_header_image();
    ...
    ?>
    <style type="text/css" id="casper-header-css">
...
function casper_sanitize_input( $input ) {
    return strip_tags( stripslashes( $input ) );
} // end casper_sanitize_input
...
add_action( 'customize_register', 'casper_customize_register', 11 );


// Call the script for live preview
function casper_customize_preview_js() {
    wp_enqueue_script( 'casper-customize-preview', get_template_directory_uri() . '/assets/js/min/customize-preview-min.js', array( 'customize-preview' ), '20150922', true );
}
