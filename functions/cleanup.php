...
function casper_remove_wp_ver_css_js( $src ) {
    if ( strpos( $src, 'ver=' ) )
        $src = remove_query_arg( 'ver', $src );
    return $src;
}

    add_filter( 'style_loader_src', 'casper_remove_wp_ver_css_js', 9999 );
    
    add_filter( 'script_loader_src', 'casper_remove_wp_ver_css_js', 9999 );

// remove injected CSS for recent comments widget
function casper_remove_wp_widget_recent_comments_style() {
    if ( has_filter( 'wp_head', 'wp_widget_recent_comments_style' ) ) {
        remove_filter( 'wp_head', 'wp_widget_recent_comments_style' );
    }
}

add_filter( 'wp_head', 'casper_remove_wp_widget_recent_comments_style', 1 );

// clean up comment styles in the head
function casper_remove_recent_comments_style() {
...
add_action('wp_head', 'casper_remove_recent_comments_style', 1);
...
// Media cleanup
  ...
  function casper_gallery_style($css) {
  return preg_replace("!<style type='text/css'>(.*?)</style>!s", '', $css);
}
  add_filter('gallery_style', 'casper_gallery_style');
...
// For some reason WordPress adds explicit width to the figure tag. That inhibits our responsive layout and the only way around that is to rewrite the entire img_caption_shortcode function.
  ...
  function casper_img_caption_shortcode ( $empty, $attr, $content ) {
  ...
  add_filter( 'img_caption_shortcode', 'casper_img_caption_shortcode', 10, 3 );

add_action('after_setup_theme', 'casper_cleanup');
?>